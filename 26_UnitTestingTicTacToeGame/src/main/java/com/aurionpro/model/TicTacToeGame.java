...
public class TicTacToeGame {
	...
	public boolean makeMove(int slot) {  // Place the current player's mark in the specified slot on the board.
		if(slot < 1 || slot > 9) {
			throw new IllegalArgumentException("Slot must be between 1 and 9");
		}
		if (board[slot - 1].equals(String.valueOf(slot))) {  // Check if the Slot is Unoccupied:
